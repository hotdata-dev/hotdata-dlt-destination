...
It accepts the same parameters as `hotdata_destination` (see below) plus `max_table_nesting` (default `1000`). It does not add `_hotdata_*` columns — it relies on dlt's `_dlt_id` for row identity. If an existing managed database is missing a declared table on a later run, it is recreated with the union of existing and required tables.
