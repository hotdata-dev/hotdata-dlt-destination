...
# An interpreter that can `import tomllib`, stdlib only from 3.11. `python3`
# carries no version guarantee, so hardcoding it made every helper below fail
# with a bare ModuleNotFoundError -- which reads as a broken checkout rather than
# a too-old interpreter. No workflow runs this script; only a human cutting a
# release does, which is why it went unnoticed.
#
# `uv` is the fallback because `prepare` already shells out to `uv lock`.
# `>=3.11` not an exact pin: an exact request downloads a managed interpreter
# when the machine's uv-visible one is 3.11 or 3.13, and fails under
# UV_PYTHON_DOWNLOADS=never. It reads like a redirect but is not -- $PY_BIN is
# expanded unquoted for word splitting, and bash does not rescan the result for
...
# Resolved inside the commands that need it, not at load: the paths that never
# touch Python must keep working without an interpreter, or the message naming
# the one you need is itself gated on having it.
PY_BIN=""
