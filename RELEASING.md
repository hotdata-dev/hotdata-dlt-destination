...
If the publish failed *before* PyPI accepted the upload — a stale action pin, a
PyPI outage — re-run it against the same tag rather than deleting the tag or
...
picked up; the `tag` input selects what gets built. Everything the run executes
...
```bash
curl -s -o /dev/null -w '%{http_code}' https://pypi.org/pypi/hotdata-dlt-destination/X.Y.Z/json
```
